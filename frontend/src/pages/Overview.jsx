...
export default function Overview() {
  ...
  return (
    <div className="page">
      ...
      {loading ? <div className="grid-4">{[1, 2, 3, 4].map((item) => <div key={item} className="skeleton" style={{ height: 138 }} />)}</div> : <div className="grid-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))" }}>{stats && Object.entries(stats).map(([key, value]) => { const config = CARD_CONFIG[key]; const Icon = config?.icon || Activity; return <div className="stat-card" key={key} style={{ position: "relative", overflow: "hidden" }}><div style={{ width: 38, height: 38, display: "grid", placeItems: "center", borderRadius: 10, color: config.color, background: config.bg }}><Icon size={18} /></div><div><div className="stat-value">{value}</div><div className="stat-label">{config.label}</div></div><div style={{ position: "absolute", bottom: 0, left: 0, height: 3, width: "100%", background: config.color, opacity: .75 }} /></div>; })}</div>}
